...
// Set exam ID to load — change this to switch exams
const EXAM_ID = 356;
...
const Index = () => {
  ...
  const { data: analyticsData, loading, error: apiError } = useExamAnalytics(EXAM_ID);
    ...
    <DashboardLayout>
      ...
      {apiError && (
        <div className="py-2 px-4 mb-4 bg-destructive/10 text-destructive rounded-xl text-sm">
          ⚠️ Không thể kết nối API: {apiError} — Hiển thị dữ liệu tĩnh.
        </div>
      )}
